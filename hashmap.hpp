...
public:
    ...
    using const_pair_type         = std::pair<const Key, Value>;    // 返回给外部的键值对
    ...
    using allocator_type          = Allocator;                      // 分配器
                                                                    ...
                                                                    // 哈希目标块获取器
    using hash_block_maker_type   = std::function<std::pair<const void *, size_type>(const void *, size_type)>;
    using hasher_type             = utils::XXHash32;
          ...
          bucket = bucket_type(
            ...
            /*
              ...
              std::cout << "a.first = " << a.first << "; "
                        << "a.second = " << a.second << std::endl;
              std::cout << "b.first = " << b.first << "; "
                        << "b.second = " << b.second << std::endl;
              std::cout << "a.first < b.first == " << (a.first < b.first) << std::endl;
              */
...
protected:
  ...
  hasher_type                   hasher; 
  hash_block_maker_type         hash_block_maker =  // 哈希目标块获取器
    [] (const void *p, size_type size) {
...
public:
  ...
  public:
    ...
    iterator() : hashmap_ptr(nullptr), 
                 current_bucket_index(0),
                 is_end_iterator(true)      { }

    iterator(HashMap* map, bool is_end)
      : hashmap_ptr(map), is_end_iterator(is_end)
    {
    ...
    find_previous_valid_element() 
    {
      ...
      while (true) {
        ...
        if (current_bucket_index > 0) {
          ...
          if ( current_box->box_map.get(current_bucket_index) && 
               current_box->box[current_bucket_index].size() > 0 ) {
            rbtree_iter = current_box->box[current_bucket_index].end();
          ...
          // 移动到上一个箱
          if (current_box == hashmap_ptr->box_list.begin()) return;
...
protected:
   ...
   */
  static size_type
  calculate_initial_box_capacity(size_type estimated_size)
  {
    ...
    size_type required_capacity =
      static_cast<size_type>(
        static_cast<double>(estimated_size) / LOAD_FACTOR_THRESHOLD
      ) + 1;
  ...
  template <typename T>
  size_type
  get_bucket_index(const T& key) const 
  {
    std::pair<const void *, size_type> ret = 
      this->hash_block_maker(&key, static_cast<size_type>(sizeof(key)));
    return
      this->hasher.hash_linear(
        ret.first, ret.second, 
        0, this->box_capacity - 1,
        0
      );
  }
  ...
  should_expand() const 
  {
    ...
    double current_factor = 
      static_cast<double>(last_box.used_bucket_count) / box_capacity;

    return current_factor > LOAD_FACTOR_THRESHOLD;
  }
   ...
   */
  const_pair_type *
  find_in_bucket(const bucket_type& bucket, const key_type& key) const
  {

    pair_type search_pair(key, value_type{});
    const pair_type* found = bucket.find(search_pair);

    return reinterpret_cast<const_pair_type *>(const_cast<pair_type*>(found));
  }
...
public:
  ...
  init()
  {
    this->hash_block_maker = [] (const void *p, size_type size) {
      return std::make_pair(p, size);
    };

    if constexpr (std::is_same<Key, std::string>::value) {
      this->hash_block_maker = 
        [](const void * ptr, size_type)
          -> std::pair<const void *, size_type> 
        {
          const std::string *sptr = static_cast<const std::string *>(ptr);
          return std::make_pair(
            sptr->c_str(),
            static_cast<size_type>(sptr->size())
          );
        };
    }
  }
   ...
   */
  explicit
  HashMap(size_type estimated_size = 0)
  {
    this->count = 0;
    this->init();
    this->box_capacity =
      this->calculate_initial_box_capacity(estimated_size);
  ...
  HashMap(InputIt     first,
          InputIt     last, 
          size_type   estimated_size = 0) 
    : HashMap(estimated_size) 
  {
    this->count = 0;
    this->init();
   ...
   * @param estimated_size 预估元素数量
   */   
  HashMap(std::initializer_list<pair_type>  init,
          size_type                         estimated_size = 0)
    : HashMap(estimated_size) 
  {
    this->count = 0;
  ...
  void
  set_hash_block_maker(hash_block_maker_type maker)
  { this->hash_block_maker = maker; }
   ...
   */
  void
  insert(std::initializer_list<const_pair_type> ilist) 
  {
  ...
  iterator 
  begin() const
  { return iterator(const_cast<HashMap*>(this), false); }   // false = 不是end迭代器
  ...
  { return iterator(const_cast<HashMap*>(this), true); }
  ...
  size_type
  max_size() const 
  { return this->box_list.size() * this->box_capacity; }
  ...
  find(const Key& key) const
  {
    ...
    for (auto& box_mgr : this->box_list) {
      if (box_mgr.box_map.get(keyhash)) {
        const_pair_type* found = find_in_bucket(box_mgr.box[keyhash], key);
        if (found) {
          // 创建指向此元素的迭代器
          iterator result;
          result.hashmap_ptr = const_cast<HashMap*>(this);
          result.is_end_iterator = false;
